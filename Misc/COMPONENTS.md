# Components

*This is a document to specify the things you need to build the project* 

## 1. Motors 

**T-Motor Velox V2207 V3 FPV Motor 2050KV**

### Where to buy:
* [Kaufland](https://www.kaufland.de/product/562724996/?kwd=&source=pla&id_unit=391965683983&gad_campaignid=23214783807)    ~ 15,50€
* [T-Hobby](https://www.t-hobby.com/products/tmotor-velox-v2207-v3-motor) ~ 14,90$ *(dont know if also for europe)*

### Specs:
- Diameter: approximately 27.5 mm
- Motor body height: approximately 30 mm
- Total height including shaft: approximately 42 mm
- Shaft diameter: 3.17 mm
- Weight: approximately 47–50 g

## 2. Propeller
**HQProp ETHiX S5 5X4X3 Cinematic Prop**

### Where to buy:
* [n-Factory.de](https://n-factory.de/HQProp-ETHiX-S5-5X4X3-Cinematic-Prop) ~ 2,95€/4 pieces *(would buy 3 sets to have some spares if one broke)*

### Specs:
- Diameter: 5 inch (≈127 mm)
- Pitch: 4.0 inch
- Blades: 3 (tri-blade design)

## 3. ESC (electric speed controller)
**4in1 V45A Lite ESC By T-Motor**

[mepsking](https://www.mepsking.shop/thobby-v45a-lite-4in1-esc.html?spec=V45A%20Lite) ~ 45,90€

### Specs:
- Input voltage : 3-6S
- Current : 4*45A
- Max current : 4*55A
- Amperage meter : 200
- Software : BLHeli_S
- Weight : 13g
- Mounting hole : 30.5mm

## 4. Battery 
**Tattu 6S 1480mAh R-Line V6**


[amazon.de](https://www.amazon.de/Tattu-1480mAh-R-Line-22-2V-Hochleistungsakku/dp/B0FCM3TYLW/ref=sxin_15_pa_sp_search_thematic_sspa?cv_ct_cx=5s+lipo&sbo=RZvfv%2F%2FHxDF%2BO5021pAnSA%3D%3D&sr=1-1-88822a8f-53d7-4131-9cc7-8d3bac8f63a7-spons&aref=9fn701CKvN&sp_csd=d2lkZ2V0TmFtZT1zcF9zZWFyY2hfdGhlbWF0aWM&psc=1)

### Specs
**Basic Specifications:**
* Voltage: 22.2 V (6S)
* Capacity: 1480 mAh
* Discharge rate: 160C (realistically approx. 35C)
* Connector type: XT60 + XH balance connector
* Weight: 220 g

**Dimensions:**
* ST version: 77 × 37.5 × 36.5 mm (elongated shape)
* SQ version: 69.5 × 42 × 36.5 mm (compact shape)


# Miscellaneous
### Heated Inserts
[Amazon.de](https://www.amazon.de/M2-5-Bolatus-Einschmelzmuttern-Einschmelzhilfe-Einschmelzgewinde/dp/B0F2TGGBMC/ref=sr_1_1_sspa?sr=8-1-spons&aref=PFkAvlBKTN&sp_csd=d2lkZ2V0TmFtZT1zcF9hdGY&language=de_DE) ~ 11,89€

### Hex Standoff, Internal/External Thread M3, 30 mm
[Pollin.de](https://www.pollin.de/p/abstandsbolzen-sechskant-innen-aussengewinde-m3-30-mm-442655?etcc_med=fshopping&etcc_ori=google&et_cmp_seg2=sea-fshopping&channelid=fshopping&etcc_med=CPC&etcc_par=Google&etcc_cmp=PMax-DE-CSS-Shopping-VP-5&et_cmp_seg2=SEA-CSS&etcc_var=Cj0KCQjw2YDQBhD_ARIsAE1qeSeSoJagWbDsTZgaPo5HA10Ewup4k2sOMOFAIl_DViiukDZVPhNNwusaAuhqEALw_wcB&et_cmp_seg1=20281414516&etcc_grp=&etcc_bky=&etcc_mty=) ~ 0,27€/piece (buy min 6.)

## Price


| Piece | Price | Quantity |
| :--- | :---: | ---: |
| Motor | 15,50€ | 4 |
| Propeller | 2,95€ | 3 |
| Heat Inserts | 11,89€ | 1 |
| M3 Standoff 30mm | 0,27€ | 6 |
| ESC | 45,90€ | 1 |

*currently round about 130€* 
